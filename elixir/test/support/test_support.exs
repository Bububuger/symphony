defmodule SymphonyElixir.TestSupport do
  ...
  defp workflow_content(overrides) do
    ...
    tracker_workflow_stages = Keyword.get(config, :tracker_workflow_stages)
    ...
    sections =
      [
        ...
        tracker_project_slugs && "  project_slugs: #{yaml_value(tracker_project_slugs)}",
        "  workspace_slug: #{yaml_value(tracker_workspace_slug)}",
        "  project_id: #{yaml_value(tracker_project_id)}",
        "  assignee: #{yaml_value(tracker_assignee)}",
        "  active_states: #{yaml_value(tracker_active_states)}",
        "  terminal_states: #{yaml_value(tracker_terminal_states)}",
        tracker_workflow_stages && "  workflow_stages: #{yaml_value(tracker_workflow_stages)}",
        "polling:",
  ...
  defp runtimes_yaml(runtimes) when is_list(runtimes) do
    entries =
      Enum.map(runtimes, fn rt ->
        ...
        lines =
          case rt[:turn_timeout_ms] || rt["turn_timeout_ms"] do
            nil -> lines
            ms -> lines ++ ["    turn_timeout_ms: #{yaml_value(ms)}"]
          end
